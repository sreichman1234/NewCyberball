...
Participants were randomly assigned to one cell of a 3 (status: high vs low vs control) X 2 (context: task vs social) between-subjects ANOVA. We collected data on the number of throws made to three different targets, creating a within-subjects target factor (target: excluder vs victim vs bystander); throws occurred both before and after an exclusion request, yielding a second within-subjects factor (time: pre vs post exclusion request). All together, this creates a mixed design with two between and two within-Ss factors.

Conditions are designated in the dataset as follows: 
- task_low
- social_low
- task_high
- social_high
...
We set out to test these hypotheses:

Hypothesis 1: 
Excluder status influences an individual’s decision of how to respond to the excluder’s request to ostracise another group member. 
  1a) Participants comply with an exclusion request by a high status excluder but won’t cooperate with requests from a low status excluder. 
  1b) Participants exclude a low status individual who attempts to exclude another group member.
  
Hypothesis 2: 
The effect of status on exclusion is moderated by context.
  2a) Observers comply more with an exclusion request in a task-oriented group than in a social context. 


## Measures

- Manipulation checks: We tested the status manipulation in a pretest.

Our measures include:

Throwing behaviour:
- The original dataset contains counts of throws along with matching timestamps and who threw it and to whom. From this, we derive **proportions** of throws to the victim (excluder), both before and after the exclusion request. Reduced proportion of throws to the victim following the request indicates compliance with the excluder.

Perceptions of the group experience:
  - **Perceived conflict type**
    1.My team members experienced relationship tension that was not related to the activity
    2. My team members had conflicting opinions about the activity we were doing
Perceptions of the excluder:
  - **Identification**
    1. {excluder} is representative of this group
    2. {excluder} identifies with this group
  - **Self vs other concern (excluder motivation)**
    1. {excluder} is concerned about their own needs and interests
    2. {excluder} is concerned about the needs and interests of other group members
  - **Norm violation** 
    1. The way {excluder} behaved was wrong
    2. ...inappropriate
    3. ...improper
    4. {excluder} acted poorly

In the inferential tests below, we use three contrast-coded predictors and their interactions.

- **x1status** = +1 (high), -1 (low), 0 (control)
- **x2status** = +1 (high, low), -2 (control)
- **x3context** = +1 (task), -1 (social)

## Analysis Strategy

- intercorrelations of scale vars and status within context
- do condition vars predict scale vars?
- 5 ANOVAs testing effects of condition on identification, motivation, norm violation, task conflict, relationship conflict
- mixed model ANOVAs (lmer) testing effects of between-Ss conditions AND within-Ss factor (time) on proportion of throws to victim, excluder (H1, H2)
  - later, we will include target as a within-Ss factor (H1, H2)
- mixed model regressions (lmer) testing effects of 5 scale measures (ID, motivation, NV, task conflict, relationship conflict) on pre-post proportion of throws to victim, excluder 
  - later, we will consider examining whether the scale measures _moderate_ or perhaps _mediate_ any relationships between our predictors and throwing behaviour.
