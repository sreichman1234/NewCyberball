...
## Study Description
This is study designed to follow up on review comments received on the rejected JAP/JPSP submission of a paper reporting this programme of research. Here, we test whether ratings of a potential leader differ (across conditions and within the group) as a function of deference--shown by nodding and smiling--from another group member. In addition to manipulating deference, we also manipulate conversation content (task versus social) and the casting of the roles (always the same 'confederate,' but different potential leaders). We requested 20 initial responses from Prolific Academic to be sure data were recorded correctly, followed by an additional 880 responses to seek N > 110 per cell. The screener  requirements were US nationality and current location (because this was specified in the CU IRB approval), 95% approval on at least 10 studies, and no past participation in any related studies; we received 957 total responses, including some partial responses, before exclusions. 


We collected N = `r nrow(studydata)` observations from Prolific Academic. After checking for ineligible countries (n = `r (nrow(checkdata) - nrow(d))`), duplicate IP addresses (`r nrow(dupefdata)`), and questionable quality-check responses (`r nrow(racestop.df)`), we retained N = `r nrow(betterdata)` observations for analysis. Respondents included `r sum(betterdata$gender == 1)` male, `r sum(betterdata$gender == 2)` female, and `r sum(betterdata$gender == 3)` self-defining participants. The average age of the respondents was  `r mean(betterdata$age)`. Data were collected in early January 2022.


Participants were randomly assigned to one cell of a 2 (follower conduct: deference vs neutral) X 2 (cast version: 1 vs 2) X 2 (conversation: task vs social) design, resulting in 8 conditions:

- Condition 1 - Cast 1, Task conversation, deference
- Condition 2 - Cast 1, Task conversation, neutral
- Condition 3 - Cast 1, Social conversation, deference
- Condition 4 - Cast 1, Social conversation, neutral
- Condition 5 - Cast 2, Task conversation, deference
- Condition 6 - Cast 2, Task conversation, neutral
- Condition 7 - Cast 2, Social conversation, deference
- Condition 8 - Cast 2, Social conversation, neutral

Following our previous studies, we seek support for the following hypotheses (numbered hypotheses are taken from our most current manuscript). The goal of this study is to replicate the general pattern of Studies 1-3 in the existing paper, to provide evidence of the generality and robustness of our effects, and to test whether our findings extend to female potential leaders.

  1. A potential leader is more likely to be judged “a leader” (e.g., through nomination to lead the group) when a follower shows extremely subtle, nonverbal deference toward him/her than when the follower shows no such deference.  
  2. A potential leader is seen as having more leader qualities (i.e., power, likability, and competence) when a follower shows extremely subtle, nonverbal deference toward him/her than when the follower shows no such deference.  

We also test an additional hypothesis unique to this study:

  3. Deference-based judgments of the potential leader do not depend on the type of conversation or on the specific individual assigned to that role. (That is, no moderation by conversation or cast.)  
...
- Manipulation checks: How much each target was **nodding** and **smiling**
- Our original **power**, **likability**, **competence** measures
- Three competence measures from Anderson & Kilduff, 2009
  - **General competence** 
    1. "skills and expertise that would allow him/her to perform well on the group task (e.g., creativity, analytical abilities)"
    2. general intelligence
    3. "leadership ability (i.e., the ability to coordinate group activities and motivate a group to pursue a common goal)"
    4. verbal skills
  - **Task competence**
    1. dependable and self-disciplined 
    2. conventional and uncreative (r)
  - **Social competence**
    1. extraverted and enthusiastic 
    2. reserved and quiet (r)
    
In the inferential tests below, we use three contrast-coded predictors and their interactions.

- **x1cond** = +1 (deference), -1 (neutral)
- **x2cast** = +1 (cast 1), -1 (cast 2)
- **x3convo** = +1 (task convo), -1 (social convo)

## Analysis Strategy

### Testing H1

We test H1 as follows:  
[From manuscript:]  
__First__, we used chi-square analysis to analyze the extent to which the PL was nominated to lead the group in the subsequent task. To do this, we compared the number of nominations of the PL versus others in the group in each condition. 
*Note, we are unable to test H1 in this study because we did not collect the nomination measure.* 
...
write.csv(trialdata1,"../experiments/trialcleandata.csv")
```
...
excluder_all.df<- rbind(HS_p1_exclusion_full, HT_p1_exclusion_full, MS_p3_exclusion_full, MT_p3_exclusion_full, LS_p4_exclusion_full, LT_p4_exclusion_full)

#victim proportions across conditions
victim_all.df<- rbind(HS_p3_exclusion_full, HT_p3_exclusion_full, MS_p4_exclusion_full, MT_p4_exclusion_full, LS_p3_exclusion_full, LT_p3_exclusion_full)
...
# dataset with excluder and victim
```{r}

victim_all.df <- victim_all.df %>% 
  mutate("player" = 1)
...
emmeans(excluder_modelexclusion, list(pairwise ~ xtime), adjust = "tukey")
 ...
 #emmeans(excluder_modelexclusion, specs = )
...
# three way int? 
interact_plot(victim_modelexclusion, pred= xtime, mod2 = x1status, modx = x3context)
...
#taken trial data and aligned columns
#read in
...
#reliability norm violation scale
...
#combine Qualtrics n Cyberball
QualtricsCyberball.df <- merge(qualtricsMCQ, victim_all.df, by = "prolific_id")

#representative excluder
representative_excluder.lm <- lm(representative ~  (x1status + x2status) * x3context , data = QualtricsCyberball.df)
summary(representative_excluder.lm)
anova(representative_excluder.lm)
###sig w x1status x2 status and context (no int)

#identification exc
identification_excluder.lm <- lm(identification ~  (x1status + x2status) * x3context , data = QualtricsCyberball.df)
summary(identification_excluder.lm)
anova(identification_excluder.lm)
#non sig 
#because they lack empathy “Player 1 identifies with other members of this group”
...
#own need excluder
own_need_excluder.lm <- lm(own_need ~  (x1status + x2status) * x3context , data = QualtricsCyberball.df)
summary(own_need_excluder.lm)
anova(own_need_excluder.lm)

# sig x1status, x3context and x1status:x3context  
...
#other need excluder
other_need_excluder.lm <- lm(other_need ~  (x1status + x2status) * x3context , data = QualtricsCyberball.df)
summary(other_need_excluder.lm)
anova(other_need_excluder.lm)
# sig x3context  


#table cor
apa.cor.table(other_need_excluder.lm,filename = "Example3.doc",table.number = 3,show.conf.interval = F)
####dont understand output of means? Theyre neg
