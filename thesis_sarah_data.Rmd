...
all_players_data <- rbind(excluder_all.df, victim_all.df, bystander_all.df)
...
#table cor
# apa.cor.table(representative_excluder.lm,filename = "Representative.doc",table.number = 3,show.conf.interval = F)
# apa.cor.table(identification_excluder.lm,filename = "Identification.doc",table.number = 3,show.conf.interval = F)
# apa.cor.table(own_need_excluder.lm,filename = "OwnNeed.doc",table.number = 3,show.conf.interval = F)
# apa.cor.table(other_need_excluder.lm,filename = "OtherNeed.doc",table.number = 3,show.conf.interval = F)
...
self_other_vars <- c("own_need","other_need")
...
#no full data has dummy coded status so im doing it now
new_data <- mutate(trialdata1, x1status =
              ifelse(status == "high", "1",
              ifelse(status == "low", "-1", "0")))

#making x1status numeric
new_data$x1status <- as.numeric(new_data$x1status)

#merging x1status and qualtrics
qualtricsMCQ <- merge(qualtricsMCQ, new_data, by = "prolific_id")

#corr with the four scales and x1 status
corrdata <- qualtricsMCQ %>%
  select(sc_iden, sc_self_other, sc_norm, sc_rel_task, x1status) 
