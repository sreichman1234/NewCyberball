...
# three way int? 
three_way_int <- interact_plot(victim_modelexclusion, pred= xtime, mod2 = x1status, modx = x3context, file = "3wayint")
...
#qualtricsMCQ$other_need_r <- 8 - qualtricsMCQ[ ,8]
qualtricsMCQ$other_need_r <- 8 - qualtricsMCQ$other_need
...
new_excluder_all.df <- merge(qualtricsMCQ, excluder_all.df, by = "prolific_id")
...
#iden
sc_iden.lmer <- lmer(prop ~ xtime * (x1status + x2status) * x3context + sc_iden_md + (xtime | prolific_id), data = new_excluder_all.df)
