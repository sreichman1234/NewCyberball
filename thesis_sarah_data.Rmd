...
#PROP - MIXED model ANOVAs (lmer) testing effects of between-Ss conditions AND within-Ss factor (time) on proportion of throws to victim, excluder (H1, H2)
...
modelnull <- lmer(prop ~  ( 1 | prolific_id), data = victim_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
summary(modelnull)
...
# models
victim_modelexclusion <- lmer(prop ~ xtime * (x1status + x2status) * x3context + (xtime | prolific_id), data = victim_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
summary(victim_modelexclusion)
anova(victim_modelexclusion)

excluder_modelexclusion <- lmer(prop ~ xtime * (x1status + x2status) * x3context + (xtime | prolific_id), data = excluder_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
summary(excluder_modelexclusion)
anova(excluder_modelexclusion)

bystander_modelexclusion <- lmer(prop ~ xtime * (x1status + x2status) * x3context + (xtime | prolific_id), data = bystander_all.df,  control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
...
options(scipen = 100)
report(victim_modelexclusion)
report_random(victim_modelexclusion)
```
...
# three way int 
vic_three_way_int <- interact_plot(victim_modelexclusion, pred= xtime, mod2 = x1status, modx = x3context, n.sd = 1,x.label = "time", y.label = "Proportion of ball throws", main.title	= "Victim", file = "victim_3wayint")

excluder_three_way_int <- interact_plot(excluder_modelexclusion, pred= xtime, mod2 = x1status, modx = x3context, n.sd = 1, 
x.label = "time", y.label = "Proportion of ball throws", main.title	
= "Excluder") + apatheme
...
bystander_three_way_int <- interact_plot(bystander_modelexclusion, pred= xtime, mod2 = x1status, modx = x3context, n.sd = 1, x.label = "time", y.label = "Proportion of ball throws", main.title	
= "Bystander", file = "bystander_3wayint")

bystander_three_way_int <- interact_plot(bystander_modelexclusion, pred= xtime, mod2 = x1status, modx = x3context, n.sd = 1, x.label = "time", y.label = "Proportion of ball throws", legend.main = "context" , main.title	
= "Bystander") + theme_apa() +
  theme(text=element_text(family='Times New Roman'),
        axis.text.y=element_text(size = 12),
        axis.text.x=element_text(size = 12) + geom_bar()) 
```
...
# can add collapse.se = TRUE,

tab_model(excluder_modelexclusion, show.se = TRUE, show.re.var = FALSE, show.r2 = TRUE, show.ci = 0.95, string.ci = "95% CI", p.val = "satterthwaite", string.est = "Estimate (SE)", dv.labels = c("Proportion of Throws to the Excluder") , pred.labels = c("Intercept", "Time (pre vs post exclusion request)", "Status (linear contrast)", "Status (quadratic contrast)", "Context (Task vs Social)", "Time * Status (linear contrast)", "Time * Status (quadratic contrast) ", "Time * Context", "Status (linear contrast) * Context", "Status (quadratic contrast) * Context" , "Time * Status (linear contrast) * Context", "Time * Status (quadratic contrast) * Context"),  file = "exc_model.html")
          


tab_model(victim_modelexclusion, show.se = TRUE, show.re.var = FALSE, show.r2 = TRUE, show.ci = 0.95, string.ci = "95% CI", p.val = "satterthwaite", string.est = "Estimate (SE)", dv.labels = c("Proportion of Throws to the Victim") , pred.labels = c("Intercept", "Time (pre vs post exclusion request)", "Status (linear contrast)", "Status (quadratic contrast)", "Context (Task vs Social)", "Time * Status (linear contrast)", "Time * Status (quadratic contrast) ", "Time * Context", "Status (linear contrast) * Context", "Status (quadratic contrast) * Context" , "Time * Status (linear contrast) * Context", "Time * Status (quadratic contrast) * Context"),  file = "vic_model.html")
...
# tab_model(victim_modelexclusion, show.re.var = FALSE, show.se = TRUE, show.r2 = TRUE, show.ci = 0.95, string.ci = "95% CI", p.val = "satterthwaite", collapse.se = TRUE, string.est = "Estimate (SE)", dv.labels = c("Exclusion")  , file = "vic_model.html")

##can get them next to each other
# excluder_victim_results_table <- tab_model(excluder_modelexclusion, victim_modelexclusion, dv.labels = c("excluder", "victim"), show.re.var = FALSE, show.se = TRUE, show.r2 = TRUE, show.ci = 0.95, string.ci = "95% CI", p.val = "satterthwaite", collapse.se = TRUE, string.est = "Estimate (SE)", file = "vic_exc_model.html")
```
...
sd(excluder_all.df$prop)
[1] 0.1625265

#new var combines status and time highstatuspre
#x-axis put new vars as x=

```

# plots by status context and time
```{r}


# New facet label names for dose variable
cap_name.labs <- c("High", "Control", "Low")
names(cap_name.labs) <- c("high", "control", "low")

# New facet label names for supp variable
cap_name_context.labs <- c("Task", "Social")
names(cap_name_context.labs) <- c("task", "social") 

#add means sd column for error bars
excluder_all_plot.df <- excluder_all.df %>%
  group_by(status, context) %>% mutate(mean = as.numeric(as.character(mean(prop))),
                     sd = as.numeric(as.character(sd(prop))))


### I tried to swtich to proportions by using gcol but was weird and said they were throwing all the time to exc across all conditions 

excluder.plot <-excluder_all.df %>%
  mutate(status = fct_relevel(status, 
            "high", "control", "low")) %>%
  mutate(time = fct_relevel(time, 
            "pre", "post")) %>%
        ggplot(aes(x=time, y=prop, fill = time)) + facet_grid(~context + ~status, labeller = labeller(status = cap_name.labs, context = cap_name_context.labs)) +
    geom_bar(stat="identity") +
    geom_line(size=0.8)  +
  scale_fill_manual(values = c("Light Grey", "Dark Grey")) + geom_errorbar(mapping=aes(prop, ymin = prop-sd, ymax = prop+sd),width=0.1,size=0.3)+
  theme_bw()+
   theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),        
        axis.text.y=element_text(size = 12),
   ...
   labs(title = "Excluder", y = "Percentage of Ball Tosses") +
  labs(fill = "Time")+ 
...
excluder.plot
 ...
 geom_errorbar(
    aes(ymin = mean-sd, ymax = mean+sd), data = df.summary, 
    width = 0.2, position = position_dodge(0.8))+

excluder.plot <-excluder_all.df %>% 
        ggplot(aes(x=time, y=prop, fill = time), position = "dodge")
excluder.plot


                                                                              excluder_all.df$mean <- excluder_all.df %>%
  group_by(status, context) %>%
  summarise(mean = mean(prop) )                                                               


merge(df.summary, excluder_all.df, by = status)                                                                              
                                                                              excluder_all.df$sd <- sd(prop)                                                                                                                                                                                                                                                        
                                                                                                                                          
df.summary <- excluder_all.df %>%
  group_by(status, context) %>%
  summarise(
...
f <- ggplot(
  df.summary, 
  aes(x = , y = prop, ymin = prop-sd, ymax = prop+sd)
  )


+geom_errorbar( aes(x=name, ymin=value-sd, ymax=value+sd), width=0.4,size=1.3)
 

ggsave("excluder.plot.png")


victim.plot <- victim_all.df %>%
  mutate(status = fct_relevel(status, 
            "high", "control", "low")) %>%
  mutate(time = fct_relevel(time, 
            "pre", "post")) %>%
        ggplot(aes(x=time, y=prop,
           fill = time), position = "dodge") + facet_grid(~context + ~status ) +
    geom_bar(stat="identity") +
    geom_line(size=0.8) + 
  apatheme +  scale_fill_manual(values = c("Light Grey","Dark Grey")) 




bystander.plot <- bystander_all.df %>%
  mutate(status = fct_relevel(status, 
            "high", "control", "low")) %>%
  mutate(time = fct_relevel(time, 
            "pre", "post")) %>%
        ggplot(aes(x=time, y=prop,
           fill = time)) + facet_grid(~context + ~status ) +  geom_bar(stat="identity") +
    geom_line(size=0.8) +
   labs(title="Bystander",
        x ="", y = "Proportion of Ball Tosses") +
  labs(fill = "Time (pre vs post exclusion request)")+
axis.title.x = element_blank()
  apatheme +  scale_fill_manual(values = c("Light Grey","Dark Grey"))  
...
REPEATqualtricsMCQ <- read_excel("//Users/sarahreichman/Documents/NewGit/NewCyberball/REPEATqualtricsAligned.xlsx")
...
qualtricsMCQ$wrong <- as.numeric(qualtricsMCQ$wrong)
qualtricsMCQ$poorly <- as.numeric(qualtricsMCQ$poorly)
qualtricsMCQ$inappropriate <- as.numeric(qualtricsMCQ$inappropriate)
qualtricsMCQ$improper <- as.numeric(qualtricsMCQ$improper)
...
qualtricsMCQ$other_need_r <- as.numeric(qualtricsMCQ$other_need_r)
...
library(rempsyc)
means_scales_table <- nice_table(means)
 save_as_docx(means_scales_table, path = "updated_means_scales_table.docx")
#updating it lead to self-other changing
```
...
```
#******@ JEN - here ******!!!!!!!!!!!!!!
# SC_IDEN EXCLUDER Code to analyse simple effects
```{r}
write_csv(qualtricsMCQ, "qualtricsMCQ.csv")


# first, deviate the status variables to test effects within levels of status

qualtricsMCQ <- mutate(qualtricsMCQ, 
...
sc_iden_excluder_simples_hs <- lm(sc_iden ~  (d2_x1statushi + d2_x2statushi) * x3context, data = qualtricsMCQ)
summary(sc_iden_excluder_simples_hs)
anova(sc_iden_excluder_simples_hs)
#context sig


sc_iden_excluder_simples_ls <- lm(sc_iden ~  (d2_x1statuslo + d2_x2statuslo) * x3context, data = qualtricsMCQ)
summary(sc_iden_excluder_simples_ls)
anova(sc_iden_excluder_simples_ls)
#

sc_iden_excluder_simples_con <- lm(sc_iden ~  (d2_x1statuscon + d2_x2statuscon) * x3context , data = qualtricsMCQ)
summary(sc_iden_excluder_simples_con)
anova(sc_iden_excluder_simples_con)
...
sc_iden_excluder_simples_social <- lm(sc_iden ~  (x1status + x2status) * d2_x3context_s , data = qualtricsMCQ)
summary(sc_iden_excluder_simples_social)
anova(sc_iden_excluder_simples_social)
  

sc_iden_excluder_simples_task <- lm(sc_iden ~  (x1status + x2status) * d2_x3context_t, data = qualtricsMCQ)
summary(sc_iden_excluder_simples_task)
anova(sc_iden_excluder_simples_task)
#x2status

# #test individual slopes - within intersections of power and context levels
# 
# #high status - social (dotted line in RH panel)
# sc_iden_excluder_simples_highsoc <- lm(sc_iden ~  (d2_x1statushi + d2_x2statushi) * d2_x3context_s, data = qualtricsMCQ)
# summary(sc_iden_excluder_simples_highsoc)
# anova(sc_iden_excluder_simples_highsoc)
# 
# ##sig intercept
# 
# #high status - task (solid line in RH panel)
# sc_iden_excluder_simples_hightask <- lm(sc_iden ~  (d2_x1statushi + d2_x2statushi) * d2_x3context_t, data = qualtricsMCQ)
# summary(sc_iden_excluder_simples_hightask)
# anova(sc_iden_excluder_simples_hightask)
# #sig
# 
# #low status - social (dotted line in LH panel)
# sc_iden_excluder_simples_lowsoc <- lm(sc_iden ~  (d2_x1statuslo + d2_x2statuslo) * d2_x3context_s , data = qualtricsMCQ)
# summary(sc_iden_excluder_simples_lowsoc)
# anova(sc_iden_excluder_simples_lowsoc)
# #sig
# 
# #low status - task (solid line in LH panel)
# sc_iden_excluder_simples_lowtask <- lm(sc_iden ~  (d2_x1statuslo + d2_x2statuslo) * d2_x3context_t , data = qualtricsMCQ)
# summary(sc_iden_excluder_simples_lowtask)
# anova(sc_iden_excluder_simples_lowtask)
#  #sig
# 
# #control status - social (dotted line in centre panel)
# sc_iden_excluder_simples_consoc <- lm(sc_iden ~  (d2_x1statuscon + d2_x2statuscon) * d2_x3context_s , data = qualtricsMCQ)
# summary(sc_iden_excluder_simples_consoc)
# anova(sc_iden_excluder_simples_consoc)
#  
# 
# #sig
# 
# #control status - task (solid line in centre panel)
# sc_iden_excluder_simples_contask <- lm(sc_iden ~  (d2_x1statuscon + d2_x2statuscon) * d2_x3context_t , data = qualtricsMCQ)
# summary(sc_iden_excluder_simples_contask)
# anova(sc_iden_excluder_simples_contask)
# #sig
```


# SC_NORM EXCLUDER Code to analyse simple effects
```{r}



#test effects within status - this will be like looking within panels of high vs medium vs low at the 2-way interactions in the panels
sc_norm_excluder_simples_hs <- lm(sc_norm ~  (d2_x1statushi + d2_x2statushi) * x3context , data = qualtricsMCQ)
summary(sc_norm_excluder_simples_hs)
anova(sc_norm_excluder_simples_hs)
 


sc_norm_excluder_simples_ls <- lm(sc_norm ~  (d2_x1statuslo + d2_x2statuslo) * x3context, data = qualtricsMCQ)
summary(sc_norm_excluder_simples_ls)
anova(sc_norm_excluder_simples_ls)
 

sc_norm_excluder_simples_con <- lm(sc_norm ~  (d2_x1statuscon + d2_x2statuscon) * x3context , data = qualtricsMCQ)
summary(sc_norm_excluder_simples_con)
anova(sc_norm_excluder_simples_con)
 

#test effects within context - this will tell us useful stuff about how the status-exclusion patterns show up within task vs social separately
#equivalent to testing one-way ANOVA of status with only solid lines or with only the dotted lines
sc_norm_excluder_simples_social <- lm(sc_norm ~  (x1status + x2status) * d2_x3context_s , data = qualtricsMCQ)
summary(sc_norm_excluder_simples_social)
anova(sc_norm_excluder_simples_social)
#x1status 

sc_norm_excluder_simples_task <- lm(sc_norm ~  (x1status + x2status) * d2_x3context_t, data = qualtricsMCQ)
summary(sc_norm_excluder_simples_task)
anova(sc_norm_excluder_simples_task)
 

#test individual slopes - within intersections of power and context levels

#high status - social (dotted line in RH panel)
# sc_norm_excluder_simples_highsoc <- lm(sc_norm ~  (d2_x1statushi + d2_x2statushi) * d2_x3context_s, data = qualtricsMCQ)
# summary(sc_norm_excluder_simples_highsoc)
# anova(sc_norm_excluder_simples_highsoc)
# 
# 
# 
# #high status - task (solid line in RH panel)
# sc_norm_excluder_simples_hightask <- lm(sc_norm ~  (d2_x1statushi + d2_x2statushi) * d2_x3context_t, data = qualtricsMCQ)
# summary(sc_norm_excluder_simples_hightask)
# anova(sc_norm_excluder_simples_hightask)
# 
# 
# #low status - social (dotted line in LH panel)
# sc_norm_excluder_simples_lowsoc <- lm(sc_norm ~  (d2_x1statuslo + d2_x2statuslo) * d2_x3context_s , data = qualtricsMCQ)
# summary(sc_norm_excluder_simples_lowsoc)
# anova(sc_norm_excluder_simples_lowsoc)
#  
# 
# #low status - task (solid line in LH panel)
# sc_norm_excluder_simples_lowtask <- lm(sc_norm ~  (d2_x1statuslo + d2_x2statuslo) * d2_x3context_t , data = qualtricsMCQ)
# summary(sc_norm_excluder_simples_lowtask)
# anova(sc_norm_excluder_simples_lowtask)
#  
# 
# #control status - social (dotted line in centre panel)
# sc_norm_excluder_simples_consoc <- lm(sc_norm ~  (d2_x1statuscon + d2_x2statuscon) * d2_x3context_s , data = qualtricsMCQ)
# summary(sc_norm_excluder_simples_consoc)
# anova(sc_norm_excluder_simples_consoc)
#  
# 
# 
# #control status - task (solid line in centre panel)
# sc_norm_excluder_simples_contask <- lm(sc_norm ~  (d2_x1statuscon + d2_x2statuscon) * d2_x3context_t , data = qualtricsMCQ)
# summary(sc_norm_excluder_simples_contask)
# anova(sc_norm_excluder_simples_contask)

```


# SC_SELF_OTHER EXCLUDER Code to analyse simple effects
```{r}



#test effects within status - this will be like looking within panels of high vs medium vs low at the 2-way interactions in the panels
sc_self_other_excluder_simples_hs <- lm(sc_self_other ~ (d2_x1statushi + d2_x2statushi) * x3context , data = qualtricsMCQ)
summary(sc_self_other_excluder_simples_hs)
anova(sc_self_other_excluder_simples_hs)
#context
# in high status context is influential, with the excluder in the social context viewed as more self concerned than the high status excluder in the task context.

sc_self_other_excluder_simples_ls <- lm(sc_self_other ~  (d2_x1statuslo + d2_x2statuslo) * x3context, data = qualtricsMCQ)
summary(sc_self_other_excluder_simples_ls)
anova(sc_self_other_excluder_simples_ls)
 

sc_self_other_excluder_simples_con <- lm(sc_self_other ~  (d2_x1statuscon + d2_x2statuscon) * x3context , data = qualtricsMCQ)
summary(sc_self_other_excluder_simples_con)
anova(sc_self_other_excluder_simples_con)
 

#test effects within context - this will tell us useful stuff about how the status-exclusion patterns show up within task vs social separately
#equivalent to testing one-way ANOVA of status with only solid lines or with only the dotted lines
sc_self_other_excluder_simples_social <- lm(sc_self_other ~  (x1status + x2status) * d2_x3context_s , data = qualtricsMCQ)
summary(sc_self_other_excluder_simples_social)
anova(sc_self_other_excluder_simples_social)

#x1status
##within social high and low are rated differently

sc_self_other_excluder_simples_task <- lm(sc_self_other ~  (x1status + x2status) * d2_x3context_t, data = qualtricsMCQ)
summary(sc_self_other_excluder_simples_task)
anova(sc_self_other_excluder_simples_task)
 

#test individual slopes - within intersections of power and context levels

#high status - social (dotted line in RH panel)
# sc_self_other_excluder_simples_highsoc <- lm(sc_self_other ~  (d2_x1statushi + d2_x2statushi) * d2_x3context_s, data = qualtricsMCQ)
# summary(sc_self_other_excluder_simples_highsoc)
# anova(sc_self_other_excluder_simples_highsoc)
# 
# 
# 
# #high status - task (solid line in RH panel)
# sc_self_other_excluder_simples_hightask <- lm(sc_self_other ~  (d2_x1statushi + d2_x2statushi) * d2_x3context_t, data = qualtricsMCQ)
# summary(sc_self_other_excluder_simples_hightask)
# anova(sc_self_other_excluder_simples_hightask)
# 
# 
# #low status - social (dotted line in LH panel)
# sc_self_other_excluder_simples_lowsoc <- lm(sc_self_other ~  (d2_x1statuslo + d2_x2statuslo) * d2_x3context_s , data = qualtricsMCQ)
# summary(sc_self_other_excluder_simples_lowsoc)
# anova(sc_self_other_excluder_simples_lowsoc)
#  
# 
# #low status - task (solid line in LH panel)
# sc_self_other_excluder_simples_lowtask <- lm(sc_self_other ~  (d2_x1statuslo + d2_x2statuslo) * d2_x3context_t , data = qualtricsMCQ)
# summary(sc_self_other_excluder_simples_lowtask)
# anova(sc_self_other_excluder_simples_lowtask)
#  
# 
# #control status - social (dotted line in centre panel)
# sc_self_other_excluder_simples_consoc <- lm(sc_self_other ~  (d2_x1statuscon + d2_x2statuscon) * d2_x3context_s , data = qualtricsMCQ)
# summary(sc_self_other_excluder_simples_consoc)
# anova(sc_self_other_excluder_simples_consoc)
#  
# 
# 
# #control status - task (solid line in centre panel)
# sc_self_other_excluder_simples_contask <- lm(sc_self_other ~  (d2_x1statuscon + d2_x2statuscon) * d2_x3context_t , data = qualtricsMCQ)
# summary(sc_self_other_excluder_simples_contask)
# anova(sc_self_other_excluder_simples_contask)
```

#not converging lmer scale
```{r}

#scale predictor - mean centred, 
...
#combining prop and xtime with MCQ
...
#iden
sc_iden.lmer <- lmer(prop ~ xtime * (x1status + x2status) * x3context + sc_iden_md + (xtime | prolific_id), data = new_excluder_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
summary(sc_iden.lmer)
anova(sc_iden.lmer)

#scaling iden
sc_iden.lmer <- lmer(prop ~ scale(xtime) * scale(x1status + x2status) * scale(x3context) + scale(sc_iden_md) + (scale(xtime )| prolific_id), data = new_excluder_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
#CONVERGING!
...
###scaling own need
sc_self_other.lmer <- lmer(prop ~ scale(xtime) * scale(x1status + x2status) * scale(x3context) + scale(sc_self_other_md) + (scale(xtime )| prolific_id), data = new_excluder_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
...
#scaling norm
sc_norm.lmer <- lmer(prop ~ scale(xtime) * scale(x1status + x2status) * scale(x3context) + scale(sc_norm_md) + (scale(xtime )| prolific_id), data = new_excluder_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
######still NOT converging
...
#scaling rel
rel_tension.lmer <- lmer(prop ~ scale(xtime) * scale(x1status + x2status) * scale(x3context) + scale(rel_tension_md) + (scale(xtime )| prolific_id), data = new_excluder_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
######still NOT converging




rel_tension.lmer <- lm(prop ~ xtime + rel_tension_md + (xtime | prolific_id), data = new_excluder_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))

```

#ANOVA POSTPROP SCALE
#
```{r}

#new data set with prop_pre and prop_post
pre_new_excluder_all.df <- new_excluder_all.df %>% filter(xtime == -1)  %>% dplyr::rename(prop_pre = prop)

post_new_excluder_all.df <- new_excluder_all.df %>% filter(xtime == 1) %>% dplyr::rename(prop_post = prop)

prop_post_only.df <- post_new_excluder_all.df %>% select(prop_post)

new_excluder_all_wide.df <- cbind(pre_new_excluder_all.df, prop_post_only.df)


#anova

rel_tension_wide.lm <- lm(prop_post ~ rel_tension_md + prop_pre, data = new_excluder_all_wide.df)
summary(rel_tension_wide.lm)
anova(rel_tension_wide.lm)

task_tension_wide.lm <- lm(prop_post ~ task_tension_md + prop_pre, data = new_excluder_all_wide.df)
summary(task_tension_wide.lm)
anova(task_tension_wide.lm)

sc_norm_wide.lm <- lm(prop_post ~ sc_norm_md + prop_pre, data = new_excluder_all_wide.df)
summary(sc_norm_wide.lm)
anova(sc_norm_wide.lm)

sc_iden_wide.lm <- lm(prop_post ~ sc_iden_md + prop_pre, data = new_excluder_all_wide.df)
summary(sc_iden_wide.lm)
anova(sc_iden_wide.lm)

sc_self_other_wide.lm <- lm(prop_post ~ sc_self_other_md + prop_pre, data = new_excluder_all_wide.df)
summary(sc_self_other_wide.lm)
anova(sc_self_other_wide.lm)




#write out
apa.aov.table(sc_iden_wide.lm, filename = "sc_iden_wide.doc", table.number = 1)
apa.aov.table(sc_self_other_wide.lm, filename = "sc_self_other_wide.doc", table.number = 1)
apa.aov.table(sc_norm_wide.lm, filename = "sc_norm_wide.doc", table.number = 1)
apa.aov.table(rel_tension_wide.lm, filename = "rel_tension_wide.doc", table.number = 1)
apa.aov.table(task_tension_wide.lm, filename = "task_tension_wide.doc", table.number = 1)
```


#prepost prop anova for victim
```{r}
#need prop and xtime
prop_vic <- victim_all.df %>% select(prolific_id, prop, time)

#combining prop and xtime with MCQ
new_victim_all.df <- merge(qualtricsMCQ, prop_vic, by = "prolific_id")

pre_new_victim_all.df <- new_victim_all.df %>% filter(time == "pre")  %>% dplyr::rename(prop_pre = prop)

post_new_victim_all.df <- new_victim_all.df %>% filter(time == "post") %>% dplyr::rename(prop_post = prop)

prop_post_only.df <- post_new_victim_all.df %>% select(prop_post)

new_victim_all.df <- cbind(pre_new_victim_all.df, prop_post_only.df)


#anova

# v_rel_tension.lm <- lm(prop_post ~ rel_tension_md + prop_pre, data = new_victim_all.df)
# summary(v_rel_tension.lm)
# anova(v_rel_tension.lm)
# 
# v_task_tension.lm <- lm(prop_post ~ task_tension_md + prop_pre, data = new_victim_all.df)
# summary(v_task_tension.lm)
# anova(v_task_tension.lm)

v_sc_norm.lm <- lm(prop_post ~ sc_norm_md + prop_pre, data = new_victim_all.df)
summary(v_sc_norm.lm)
anova(v_sc_norm.lm)

v_sc_iden.lm <- lm(prop_post ~ sc_iden_md + prop_pre, data = new_victim_all.df)
summary(v_sc_iden.lm)
anova(v_sc_iden.lm)

v_sc_self_other.lm <- lm(prop_post ~ sc_self_other_md + prop_pre, data = new_victim_all.df)
summary(v_sc_self_other.lm)
anova(v_sc_self_other.lm)

```

#attempts w convergence cont.
```{r}
#scaling task
task_tension.lmer <- lmer(prop ~ scale(xtime) * scale(x1status + x2status) * scale(x3context) + scale(task_tension_md) + (scale(xtime )| prolific_id), data = new_excluder_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=5e5)))
######still NOT converging
#POMS
#make sc range from 0 to max, divide scores by max value 
(obs-min)/ (max-min)

new_excluder_all.df$POMS_rel_tension <-(new_excluder_all.df$rel_tension_md - 1)/ (5-1)

POMS_rel_tension.lmer <- lmer(prop ~ xtime *  (x1status + x2status) * x3context + POMS_rel_tension + (xtime | prolific_id), data = new_excluder_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
## NOT CONVERGING 
...
```
# BYSTANDER Code to analyse simple effects
```{r}
...
bystander_all.df <- mutate(bystander_all.df, 
                      d2_x1statushi = x1status - 1, #high status
                      d2_x2statushi = x2status - 1,
                      d2_x1statuslo = x1status + 1, #low status
                      d2_x2statuslo = x2status - 1,
...
#original model
bystander_modelexclusion <- lmer(prop ~ xtime * (x1status + x2status) * x3context + (xtime | prolific_id), data = bystander_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
summary(bystander_modelexclusion)
anova(bystander_modelexclusion)
...
#test effects within status - this will be like looking within panels of high vs medium vs low at the 2-way interactions in the panels
bystander_simples_hs <- lmer(prop ~ xtime * (d2_x1statushi + d2_x2statushi) * x3context + (xtime | prolific_id), data = bystander_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
summary(bystander_simples_hs)
anova(bystander_simples_hs)
#xtime:x3context  sig


bystander_simples_ls <- lmer(prop ~ xtime * (d2_x1statuslo + d2_x2statuslo) * x3context + (xtime | prolific_id), data = bystander_all.df)
summary(bystander_simples_ls)
anova(bystander_simples_ls)
#time sig

bystander_simples_con <- lmer(prop ~ xtime * (d2_x1statuscon + d2_x2statuscon) * x3context + (xtime | prolific_id), data = bystander_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
summary(bystander_simples_con)
anova(bystander_simples_con)
#non sig

#test effects within context - this will tell us useful stuff about how the status-exclusion patterns show up within task vs social separately
#equivalent to testing one-way ANOVA of status with only solid lines or with only the dotted lines
bystander_simples_social <- lmer(prop ~ xtime * (x1status + x2status) * d2_x3context_s + (xtime | prolific_id), data = bystander_all.df)
summary(bystander_simples_social)
anova(bystander_simples_social)
#time, x1status , x2 status, xtime:x1status

bystander_simples_task <- lmer(prop ~ xtime * (x1status + x2status) * d2_x3context_t + (xtime | prolific_id), data = bystander_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
summary(bystander_simples_task)
anova(bystander_simples_task)
#x1status , x2 status,

#test individual slopes - within intersections of power and context levels

#high status - social (dotted line in RH panel)
bystander_simples_highsoc <- lmer(prop ~ xtime * (d2_x1statushi + d2_x2statushi) * d2_x3context_s + (xtime | prolific_id), data = bystander_all.df)
summary(bystander_simples_highsoc)
anova(bystander_simples_highsoc)
#time 


#high status - task (solid line in RH panel)
bystander_simples_hightask <- lmer(prop ~ xtime * (d2_x1statushi + d2_x2statushi) * d2_x3context_t + (xtime | prolific_id), data = bystander_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
summary(bystander_simples_hightask)
anova(bystander_simples_hightask)


#low status - social (dotted line in LH panel)
bystander_simples_lowsoc <- lmer(prop ~ xtime * (d2_x1statuslo + d2_x2statuslo) * d2_x3context_s + (xtime | prolific_id), data = bystander_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
summary(bystander_simples_lowsoc)
anova(bystander_simples_lowsoc)
#time sig 

#low status - task (solid line in LH panel)
bystander_simples_lowtask <- lmer(prop ~ xtime * (d2_x1statuslo + d2_x2statuslo) * d2_x3context_t + (xtime | prolific_id), data = bystander_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
summary(bystander_simples_lowtask)
anova(bystander_simples_lowtask)

#control status - social (dotted line in centre panel)
bystander_simples_consoc <- lmer(prop ~ xtime * (d2_x1statuscon + d2_x2statuscon) * d2_x3context_s + (xtime | prolific_id), data = bystander_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
summary(bystander_simples_consoc)
anova(bystander_simples_consoc)
#time


#control status - task (solid line in centre panel)
bystander_simples_contask <- lmer(prop ~ xtime * (d2_x1statuscon + d2_x2statuscon) * d2_x3context_t + (xtime | prolific_id), data = bystander_all.df, control = lmerControl(optimizer = "bobyqa", optCtrl=list(maxfun=2e5)))
summary(bystander_simples_contask)
anova(bystander_simples_contask)
...
darfur.sensitivity <- sensemakr(model = excluder_modelexclusion, 
                                treatment = "x1status",
                                benchmark_covariates = "x3context",
                                kd = 1:3,
                                ky = 1:3, 
                                q =x 1,
                                alpha = 0.05, 
...
#numeric
REPEATqualtricsMCQ$representative <- as.numeric(REPEATqualtricsMCQ$representative)
REPEATqualtricsMCQ$identification <- as.numeric(REPEATqualtricsMCQ$identification)
REPEATqualtricsMCQ$own_need <- as.numeric(REPEATqualtricsMCQ$own_need)
REPEATqualtricsMCQ$other_need <- as.numeric(REPEATqualtricsMCQ$other_need)
#already num rel task


#ADDED
REPEATqualtricsMCQ$wrong <- as.numeric(REPEATqualtricsMCQ$wrong)
REPEATqualtricsMCQ$poorly <- as.numeric(REPEATqualtricsMCQ$poorly)
REPEATqualtricsMCQ$inappropriate <- as.numeric(REPEATqualtricsMCQ$inappropriate)
REPEATqualtricsMCQ$improper <- as.numeric(REPEATqualtricsMCQ$improper)

# Reverse scores in the desired columns 
#REPEATqualtricsMCQ$other_need_r <- 6 - REPEATqualtricsMCQ[ ,8]
REPEATqualtricsMCQ$other_need_r <- 6 - REPEATqualtricsMCQ$other_need

#ADDED
REPEATqualtricsMCQ$other_need_r <- as.numeric(REPEATqualtricsMCQ$other_need_r)
...
REPEATqualtricsMCQ <- REPEATqualtricsMCQ %>% 
  mutate(sc_iden = rowMeans(REPEATqualtricsMCQ[ ,iden_vars], na.rm = TRUE),
         sc_self_other= rowMeans(REPEATqualtricsMCQ[ ,self_other_vars], na.rm = TRUE),
         sc_norm = rowMeans(REPEATqualtricsMCQ[ ,norm_vars], na.rm = TRUE),
         sc_rel_task = rowMeans(REPEATqualtricsMCQ[ ,rel_task_vars], na.rm = TRUE))
```
...
#norm violation scale
norm_violation.sc <- REPEATqualtricsMCQ[,9:12] 
##reliability 
norm_violation.alpha <- psych::alpha(norm_violation.sc, check.keys = TRUE)


#iden scale
iden.sc <- REPEATqualtricsMCQ[,5:6] 
##reliability 
iden.alpha <- psych::alpha(iden.sc, check.keys = TRUE)
...
#self_other scale
self_other.sc <- REPEATqualtricsMCQ[ ,self_other_vars]
##reliability 
self_other.alpha <- psych::alpha(self_other.sc, check.keys = TRUE)

apa.cor.table(REPEATqualtricsMCQ[ ,self_other_vars], filename = "REPEATself_other_corr.doc")
#.34 corr sig
##LOW RELIABILITY =0.5
########CHANGED!!!!!!!!!!!!!!!!!!?
...
#rel_task scale
rel_task.sc <- REPEATqualtricsMCQ[,3:4] 
##reliability 
rel_task.alpha <- psych::alpha(rel_task.sc, check.keys = TRUE)
...
#no full data has dummy coded status or context so im doing it now
#       x1status
REPEATqualtricsMCQ <- mutate(REPEATqualtricsMCQ, x1status =
              ifelse(status == "high", "1",
              ifelse(status == "low", "-1", "0")))

#     x2status = +1 HS, -2 C, +1 LS
REPEATqualtricsMCQ <- mutate(REPEATqualtricsMCQ, x2status =
              ifelse(status == "high", "1",
              ifelse(status == "low", "1", "-2")))

#    x3type = +1 Task, -1 Social
REPEATqualtricsMCQ <- mutate(REPEATqualtricsMCQ, x3context =
                              ifelse(context == "task", "1", "-1"))
#making numeric
REPEATqualtricsMCQ$x3context <- as.numeric(REPEATqualtricsMCQ$x3context)
REPEATqualtricsMCQ$x1status <- as.numeric(REPEATqualtricsMCQ$x1status)
REPEATqualtricsMCQ$x2status <- as.numeric(REPEATqualtricsMCQ$x2status)
...
```{r}
#corr with the four scales and x1 status
REPEATcorrdata <- REPEATqualtricsMCQ %>%
  select(sc_iden, sc_self_other, sc_norm, sc_rel_task, x1status, x3context)
           
REPEATtask_corrdata <- subset(REPEATcorrdata, corrdata$x3context=="1")

REPEATsocial_corrdata <- subset(REPEATcorrdata, corrdata$x3context=="-1")
 

apa.cor.table(REPEATtask_corrdata, filename = "REPEATtask_corr_status.doc")
apa.cor.table(REPEATsocial_corrdata, filename = "REPEATsocial_corr_status.doc")
