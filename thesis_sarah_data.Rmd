...
df2 <- melt(excluder_all.df, id.vars='time')
...
excluder.plot <- excluder_all.df %>%
  ...
  mutate(time = fct_relevel(time, 
        ...
        ggplot(aes(x=status, y=prop,
           fill = time), position = "dodge") + facet_grid(~context + ~time ) +
    geom_bar(stat="identity") +
    geom_line(size=0.8) +
  apatheme +  scale_fill_manual(values = c("Dark Grey","Light Grey")) 
...
victim.plot <- victim_all.df %>%
  ...
  mutate(time = fct_relevel(time, 
            "pre", "post")) %>%
        ggplot(aes(x=status, y=prop,
           fill = time)) + facet_grid(~context + ~time ) +
    geom_bar(stat="identity") +
    geom_line(size=0.8) +
  apatheme +  scale_fill_manual(values = c("Dark Grey","Light Grey")) 


bystander.plot <- victim_all.df %>%
  mutate(status = fct_relevel(status, 
            "high", "control", "low")) %>%
  mutate(time = fct_relevel(time, 
            "pre", "post")) %>%
...
#test effects within status - this will be like looking within panels of high vs medium vs low at the 2-way interactions in the panels
sc_self_other_excluder_simples_hs <- lmer(sc_self_other ~ xtime * (d2_x1statushi + d2_x2statushi) * x3context , data = qualtricsMCQ)
