...
### I tried to switch to proportions by using gcol but was weird and said they were throwing all the time to exc across all conditions 
...
excluder_all.df %>%
  ...
  mutate(time = fct_relevel(time, 
    ...
    geom_errorbar(mapping=aes(prop, ymin = prop-sd, ymax = prop+sd),width=0.1,size=0.3)+
    geom_line(size=0.8)  +
...
#add means sd column for error bars
victim_all.df <- victim_all.df %>% group_by(status, context) %>% mutate(sd = as.numeric(as.numeric(sd(prop))))
...
victim_all.df %>%
  group_by(context, status, time, prop) %>% 
  summarise(mean_prop = mean(prop),
            sd_prop = sd(prop)) %>%
   ggplot(aes(x=time, y=prop, fill = time)) + facet_grid(~context + ~status, labeller = labeller(status = cap_name.labs, context = cap_name_context.labs)) +
    geom_col() +
  geom_errorbar(mapping=aes(prop, ymin = mean_prop-sd_prop, ymax = mean_prop+sd_prop), width = .9)+
  scale_fill_manual(values = c("Light Grey", "Dark Grey")) + 
  theme_bw()+
   theme(panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),        
       ...
       strip.background = element_blank(),
        axis.title.x = element_blank()) +
   labs(title = "Victim", y = "Percentage of Ball Tosses") +
  labs(fill = "Time")
...
victim_plot_social 
 ...
 victim_plot_social %>%
        ggplot(aes(x=time, y=prop,
           fill = time)) + facet_grid(~status) +
    geom_col() +
   geom_errorbar(mapping=aes(prop, ymin = mean-sd, ymax = mean+sd)) +
    geom_line(size=0.8) + 
scale_fill_manual(values = c("Light Grey","Dark Grey")) 
